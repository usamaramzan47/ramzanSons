...
import { deleteProduct, fetchProducts, resetError } from '../../features/products/PorductSlice';
...
function Products() {
    ...
    const handleDelete = () => {
        if (enteredProductName === delProd.product_name) {
            // Dispatch the delete action
            dispatch(deleteProduct(delProd.product_id));
            handleClearError();
                        ...
                        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                {productsData.length > 0 && productsData.map((item) => (
                                    <tr key={item.product_id} className="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                        <td className="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">{item.product_id}</td>
                                        <th scope="row" className="flex items-center px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            <img src={item.img} alt="iMac Front Image" className="w-auto h-8 mr-3 rounded-full" />
                                            {item.product_name}
                                        </th>
                                        <td className="px-4 py-2">
                                            <span className="bg-primary-100 text-primary-800 text-xs font-medium px-2 py-0.5 rounded dark:bg-primary-900 dark:text-primary-300">{item.size}</span>
                                        </td>
                                        ...
                                        <td className="flex items-center justify-center space-x-3">
                                            <Link to="/editProduct" state={{ item }}>
                                            ...
                                            {/* Delete icon */}
                                            <svg onClick={() => { setDelProd(item); setisOpenDel(true) }} className="cursor-pointer bg-red-100 m-2 hover:bg-red-200 hover:scale-50 transition-all duration-300 rounded-md px-2 " xmlns="http://www.w3.org/2000/svg" height="35px" viewBox="0 -960 960 960" width="35px" fill="#EA3323">
            ...
            {/* delete pop up  */}
            {
                isOpenDel && <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-50">
                    <div className="bg-white p-6 rounded shadow-lg w-96">
                        ...
                        <p className="mb-4">To confirm deletion of the product <strong>{delProd.product_name}</strong>, please enter the product name below:</p>
                        <input
                            type="text"
